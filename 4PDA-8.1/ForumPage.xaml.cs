...
namespace _4PDA
{
    public sealed partial class ForumPage : Page
    {
        ...
        public ForumPage()
        {
            ...
            {
                // В Windows 8.1 заголовок User-Agent иногда нельзя изменить; запрос всё равно выполняем.
            }
        }
        ...
        private async Task LoadForumAsync(bool forceReload)
        {
            ...
            // Важно для скорости: сначала мгновенно показываем готовый кэш страницы.
            // Структуру подразделов и HTML обновляем уже после первого отображения.
        ...
        private async Task<List<ForumStructureItem>> LoadForumStructureItemsAsync(bool forceReload)
        {
            ...
            // Сетевую загрузку полной структуры форума не делаем при обычном открытии раздела:
            // она может быть заметно медленнее самого showforum и блокировать первый показ.
            // Обновляем структуру только по явному Refresh.
            if (forceReload)
            {
                try
                {
        ...
        private static ForumData ParseForumPageFast(string html, string forumId, int currentStart)
        {
            ...
            // Максимально быстрый путь: только линейные IndexOf-проходы, без Regex.Matches по всему HTML.
        ...
        private static List<ForumListItem> ParseTopicsUltraFast(string html)
        {
            ...
            // Полная/desktop-разметка 4PDA идет таблицей ipbtable:
            // Название темы | Ответов | Автор | Просмотров | Последнее сообщение.
            // В такой структуре нельзя искать автора по первой ссылке showuser рядом с названием,
            // потому что рядом находятся ссылки на непрочитанное, вложения, пагинация и последний пост.
            // Надежный источник автора — отдельная 5-я ячейка строки темы.
        ...
        private static TopicTitleLink ExtractTopicTitleLinkFromTableRowUltraFast(string row, string id)
        {
            ...
            // В desktop-таблице настоящая ссылка темы имеет id="tid-link-{topicId}".
            // Ссылки getnewpost/getlastpost тоже содержат showtopic, но это не название темы.
        ...
        private static bool IsPinnedTableTopicRowUltraFast(string html, int rowStart, string row)
        {
            ...
            // В полной таблице 4PDA закрепленные темы обычно идут после строки-разделителя
            // "Важные темы" и до строки "Темы форума". Проверяем ближайший заголовок секции сверху.
        ...
        private static string ExtractTopicInfoUltraFast(string html)
        {
            ...
            // Автор в мобильной разметке 4PDA находится в span.topic_desc:
            // "автор: <a ...showuser=...>Name</a>". Нельзя брать первую
            // ссылку showuser из всего окна, потому что туда может попасть соседняя тема
            // или ссылка автора последнего сообщения.
