...
namespace _4PDA
{
    public sealed class QmsService
    {
        ...
        public async Task SendMessageAsync(string contactId, string threadId, string message)
        {
            ...
            {
                // Старый движок QMS иногда принимает тот же xhr body, но с другим именем action.
        ...
        private static List<QmsMessage> ParseMessages(string html, string contactNick, string threadTitle, string contactAvatarUrl)
        {
            ...
            // Основная QMS-разметка 4PDA такая же, как в ForPDA:
            // <div class="date">...</div>
            // <div class="list-group-item ... our-message" data-message-id="..." data-unread-status="..."> ... <div class="msg-content">...</div>
            // ВАЖНО: принадлежность сообщения берем только из класса our-message.
            ...
            // Запасной путь для старой/нестандартной разметки. Здесь даты не выдумываем.
        ...
        private static bool DetectOwnMessage(string block, string contactNick)
        {
            ...
            // Главный признак QMS 4PDA: класс our-message. Без него list-group-item считается сообщением собеседника.
