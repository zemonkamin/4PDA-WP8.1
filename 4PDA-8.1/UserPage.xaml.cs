...
namespace _4PDA
{
    public sealed partial class UserPage : Page
    {
        ...
        {
            Type mainPageType = ResolveMainPageType();

            if (mainPageType != null)
            {
                Frame.Navigate(mainPageType, "messages");
                return;
            }
        ...
        private static Type ResolveMainPageType()
        {
            try
            {
                Assembly assembly = typeof(UserPage).GetTypeInfo().Assembly;

                Type type = assembly.GetType("_4PDA.MainPage");
                if (type != null)
                    return type;

                type = assembly.GetType("_4PDA_8._1.MainPage");
                if (type != null)
                    return type;
            }
            catch
        ...
        private async Task LogoutAndBackAsync()
        {
            ...
            try
            {
                // LogoutAsync делает серверный logout, чистит сохранённые данные аккаунта и logout key.
                await ForumAuthService.LogoutAsync();

                ResetNavigationStack();

                // По требованию кнопка «выйти» не возвращает на LoginPage/MainPage, а закрывает приложение.
            ...
            catch
            {
                try
                {
                    // Даже если серверный logout не сработал, локальное состояние надо сбросить.
