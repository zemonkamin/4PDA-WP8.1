...
namespace _4PDA
{
    ...
    public static class LiveTileService
    {
        ...
        public static void UpdateFromNewsAndQms(IEnumerable<MainPage.FeedItem> newsItems, IEnumerable<MainPage.QmsNode> qmsRoots)
        {
        ...
        private static List<LiveTileNewsItem> ConvertNews(IEnumerable<MainPage.FeedItem> newsItems)
        {
            List<LiveTileNewsItem> result = new List<LiveTileNewsItem>();
            if (newsItems == null)
                return result;

            foreach (MainPage.FeedItem item in newsItems)
            {
                if (item == null || String.IsNullOrWhiteSpace(item.Title))
                    continue;

                LiveTileNewsItem tileItem = new LiveTileNewsItem();
                tileItem.Title = CleanText(item.Title);
                tileItem.Description = CleanText(item.Description);
                tileItem.ImageUrl = NormalizeUrl(item.ImageUrl);
                result.Add(tileItem);
        ...
        private static LiveTileQmsInfo ConvertQms(IEnumerable<MainPage.QmsNode> qmsRoots)
        {
            ...
            foreach (MainPage.QmsNode node in qmsRoots)
            {
                if (node == null)
                    continue;

                int unread = node.UnreadCount;
                if (unread <= 0)
                    continue;
        ...
        private static async Task<List<LiveTileNewsItem>> LoadNewsAsync()
        {
            ...
            foreach (XElement item in document.Descendants("item"))
            {
                ...
                result.Add(new LiveTileNewsItem
                {
                    Title = title,
        ...
        private static TileNotification CreateNewsImageTileNotification(LiveTileNewsItem item, string localImage, int index)
        {
            ...
            string image = String.IsNullOrWhiteSpace(localImage) ? "ms-appx:///Assets/Logo.scale-240.png" : localImage;
